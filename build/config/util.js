...
async function getNetIp(req) {
    ...
    try {
        const baiduApi = got_1.default.get(`https://sp0.baidu.com/8aQDcjqpAAV3otqbppnN2DJv/api.php?query=${ip}&co=&resource_id=6006&t=1555898284898&ie=utf8&oe=utf8&format=json&tn=baidu`);
        const ipApi = got_1.default.get(`http://ip-api.com/json/${ip}?lang=zh-CN`);
        const [{ data }, { country, regionName, city }] = await (await Promise.all([baiduApi, ipApi])).map((x) => JSON.parse(x.body));
